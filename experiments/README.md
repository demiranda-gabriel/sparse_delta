...
|---|---|---|---|---|---|
| 5 | 2026-05-25 | [5-composite_staged](5-composite_staged/) | done-success | Staged-from-scratch composite. **Three sequential stages:** (1) M0 standalone (l_max=1, num_layers=2, tensor_features=32), freeze; (2) warm-start composite with `gate_mode=constant, λ=1.0`, M0 frozen, train **TensorTrackAdapter + M1 (l_max=3, num_layers=3, tensor_features=32)**; (3) learned gate, M0+M1+adapter frozen. New `m1_l_max > m0_l_max` path via adapter + dedicated M1 edge SH. Multi-partition spread. **Stage 2 test forces_mae=0.073 eV/Å (beats exp 4's 0.089)**; stage 3=0.096 (sparsity_coeff=1.0 too aggressive; sweep planned). | [stage1](https://wandb.ai/demiranda-gabriel/sparse-delta/runs/vrjc4hiv) / [stage2](https://wandb.ai/demiranda-gabriel/sparse-delta/runs/ay5b5zes) / [stage3](https://wandb.ai/demiranda-gabriel/sparse-delta/runs/vzqofs9q) |
| 4 | 2026-05-17 | [4-composite_warmstart](4-composite_warmstart/) | done-success | First training run of the from-scratch warm-start composite. M0 and M1 are both fresh `Allegro_Module`s; M1's `tensor_features_in_field` is aliased to M0's `_allegro_pre_final_tp_out`. Joint training; nothing frozen. FASRC `kozinsky_gpu`. **PT2 compile NaN bisect** localised failure to `ForceStressOutput` double-backward; production switched to `compile_mode: eager`. Production run (job 13516824, 200 epochs, 1h22m) test `forces_mae=0.089 eV/Å`, `per_atom_energy_mae=0.0063 eV/atom`. | [9ql3tk6c](https://wandb.ai/demiranda-gabriel/sparse-delta/runs/9ql3tk6c) |
