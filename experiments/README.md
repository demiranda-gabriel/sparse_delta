...
|---|---|---|---|---|---|
| 5 | 2026-05-25 | [5-composite_staged](5-composite_staged/) | planned | Staged-from-scratch composite. **Three sequential stages:** (1) train M0 (l_max=1, num_layers=2) standalone, freeze; (2) warm-start composite with `gate_mode=constant, λ=1.0`, M0 frozen, train **TensorTrackAdapter + M1 (l_max=2)**; (3) learned gate, M0+M1 frozen. New `m1_l_max > m0_l_max` path via adapter + dedicated M1 edge SH. SLURM-chained on `kozinsky_gpu`. | _filled per stage_ |
