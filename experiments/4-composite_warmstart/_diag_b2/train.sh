...
#SBATCH --mem=32G
#SBATCH --time=2:00:00
#SBATCH -o /n/netscratch/kozinsky_lab/Lab/demiranda/projects/sparse_delta/experiments/4-composite_warmstart/_diag_b2/logs/%x_%j.out
#SBATCH -e /n/netscratch/kozinsky_lab/Lab/demiranda/projects/sparse_delta/experiments/4-composite_warmstart/_diag_b2/logs/%x_%j.err
...
mkdir -p experiments/4-composite_warmstart/_diag_b2/logs
...
LAST_CKPT="$PROJECT_ROOT/runs/4-composite_warmstart_diag_b2/last.ckpt"
if [[ -f "$LAST_CKPT" ]]; then
...
srun .venv/bin/nequip-train \
    -cd "$PROJECT_ROOT/experiments/4-composite_warmstart/_diag_b2" \
    -cn config \
    $CKPT_OVERRIDE
